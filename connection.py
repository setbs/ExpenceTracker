...
class Data: 
    def __init__(self): #initialize file
        super(Data, self).__init__()    #making it root 
        self.create_connection()

    def  create_connection(self):  # creating connection to database sqlite
        db = QtSql.QSqlDatabase.addDatabase("QSQLITE") #creating db
        db.setDatabaseName('expense_db.db') #naming db 
        if not db.open():  # we`ll write a massage if open failed`
            QtWidgets.QMessageBox.critical(None, "cannot open database. Click cancel to exit.", QtWidgets.QMessageBox.Cancel)
            return False
        
        query = QtSql.QSqlQuery()
        query.exec("CREATE TABLE IF NOT EXISTS expenses (ID integer primary key AUTOINCREMENT, Date VARCHAR(20),"   #creating db tables
    ...
    def total_entertaiment(self):
        return self.get_total(column='Balance', filter='Category', value='Entertaiment')
